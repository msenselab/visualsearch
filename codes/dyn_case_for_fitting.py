...
def fit_subject(sub_data):
    ...
    if use_BO:
        bounds = [{'name': 'sigma', 'type': 'continuous', 'domain': (1e-3, 2)}]
        max_iter = 1
...
def get_likelihood_N(data, sim_rt, reward):
    ...
    p_like_pres = len(pres_rts_0)/(len(pres_rts_0)+len(pres_rts_1)) * np.sum(pres_sim_rt_dist.pdf(pres_rts_0)) + \
                            len(pres_rts_1)/(len(pres_rts_0)+len(pres_rts_1)) * np.sum(pres_sim_rt_dist.pdf(pres_rts_1))
    ...
    p_like = (1-lapse)*p_like_pres+(lapse/2)*np.exp(-reward/temp) + (1-lapse)*p_like_abs+(lapse/2)*np.exp(-reward/temp)
    return p_like
