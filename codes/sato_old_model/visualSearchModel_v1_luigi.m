function visualSearchModel_v1_luigi()
...
c   = 0.003;        % Cost per time (cognitive effort etc.)
...
at = @(t,N,leak) pt(t,N,leak) ./ Nt(t,N,leak);                         % Probability of observing target at t+1 (Nt depends on case)
ap = @(t,N,leak) at(t,N,leak) .* pt(t+1,N,leak);                       % LHS of optimal stopping time inequality
...
% Accumulated likelihood of observing no target in a "present" trial 
switch leak
    case 1	% Dinamic task
        q = (1-1/N).^t;
    case 0	% Static task
