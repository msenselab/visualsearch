...
def simulate_observer(arglist):
    """
    Simulates a single observer for a given condition C in (Absent, Present)

    Due to constraints by the multiprocessing module, can only take a list of aruments, rather than
    individual arguments. Arglist is composed of the following, in order:

    T, dt, g_values, mu, sigma, decisions, C

    See ObserverSim class for explanation of variables
    """
    T, dt, g_values, mu, sigma, decisions, C = arglist
    if C != 1 and C != 0:
...
class ObserverSim:
    def __init__(self, T, dt, g_values, sigma, mu, decisions, numsims=5000, **kwargs):
        ...
        arglists = list(it.product([T], [dt], [g_values],
                                   [mu], [sigma], [decisions], presence_list))
        pool = mulpro.Pool(processes=4)
        observer_responses = pool.map(simulate_observer, arglists)
        pool.close()
